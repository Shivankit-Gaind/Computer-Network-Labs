...
# define PER .1 // Packet Error Rate
				...
				case 0:
						{
								...
								if (rand() % 100 > 100 * (1 - PER))
										FLAG = 0; //FLAG=0 data pkt lost
								else
										FLAG = 1; //FLAG =1 data pkt received 

								if (rcv_pkt.sq_no == 0 && FLAG) {
				...
				case 1:
						{
								...
								if (rcv_pkt.sq_no == 1) {
										...
										printf("Invalid packet seq. no. .....Waiting for seq. no. 1\n");
								state = 1;
								break;
						}
				}
